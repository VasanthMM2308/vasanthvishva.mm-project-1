...
class Face:
    def __init__(self, root):
       ...
       b2 = Button(b2, text="Staff Face Recognition", command=self.staff_face_recog,
                   font=("time new roman", 15, "bold"), bg="darkgreen", fg="blue")
       b2.place(x=0, y=500, width=280, height=40)
       # attendace
    def mark_attendance_student(self, i, r,n ,d):
    ...
    def mark_attendance_staff(self, i,n ,d):
        with open("Staff.csv","r+",newline="\n")as f:
            mydatalist=f.readlines()
            name_list=[]
            for line in mydatalist:
                entry=line.split((","))
        ...
        def draw_boundray(img, classifier, scaleFactor, minNeighbors, color, text, clf):
            ...
            for (x, y, w, h) in features:
                ...
                if confidence > 77:
                    ...
                    self.mark_attendance_student(r,n,d)
    ...
    def staff_face_recog(self):
        def staff_draw_boundray(img, classifier, scaleFactor, minNeighbors, color, text, clf):
            gray_image = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
            features = classifier.detectMultiScale(gray_image, scaleFactor, minNeighbors)
            ...
            for (x, y, w, h) in features:
                ...
                my_cursor.execute("select Staff_Name from staff where Emp_Id_No=" + str(id))
                n = my_cursor.fetchone()
                n = str(n)
                ...
                my_cursor.execute("select Dep from staff where Emp_Id_No=" + str(id))
                d = my_cursor.fetchone()
                d = str(d)

                my_cursor.execute("select Emp_Id_No from staff  where Emp_Id_No=" + str(id))
                ...
                if confidence > 77:
                    ...
                    cv2.putText(img, f"Name:{n}", (x, y - 30), cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 255, 255), 3)
                    cv2.putText(img, f"Department:{d}", (x, y - 5), cv2.FONT_HERSHEY_COMPLEX, 0.8, (0, 255, 255), 3)
                    self.mark_attendance_staff(i,n,d)
        ...
        def recognize(img, clf, faceCascade):
            coord = staff_draw_boundray(img, faceCascade, 1.1, 10, (255, 25, 255), "Face", clf)
            return img

        faceCascade = cv2.CascadeClassifier("haarcascade_frontalface_default.xml")
        clf = cv2.face.LBPHFaceRecognizer_create()
        clf.read("Staff_classifier.xml")
